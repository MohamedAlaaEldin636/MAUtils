...
- [Currently stable modules](#currently-stable-modules-)
    - [gson & gson_annotation & gson_processor modules](#gson--gson_annotation--gson_processor-modules-)
        - [Features](#features--gson--gson_annotation--gson_processor-modules--)
        - [Download](#download--gson--gson_annotation--gson_processor-modules--)
        - [How to use](#how-to-use--gson--gson_annotation--gson_processor-modules--)
            - [Simple Use Cases](#simple-use-cases--gson--gson_annotation--gson_processor-modules--)
            - [Complex Use Cases](#complex-use-cases--gson--gson_annotation--gson_processor-modules--)
            - [All Cases](#all-cases--gson--gson_annotation--gson_processor-modules--)
...
## gson & gson_annotation & gson_processor modules [▴](#currently-stable-modules-)

- Didn't separate the base module from it's annotation & processor ones since
mostly they are all needed together & rarely used without them. 
- Several approaches differs depending on if the caller is a java or a kotlin consumer code,
So for java consumer code check out the collapsed **Java Consumer Code** Word
for how to use in java which is usally after kotlin how to use examples isa.

### Features ( gson & gson_annotation & gson_processor ) [▴](#gson--gson_annotation--gson_processor-modules-)
...
this is only where annoation is needed and only for root type, Not needed for nested ones.
3. (kotlin) object keyward(class) conversion maintain same instance.
4. support conversion for above cases even if they are inside another class as a property.
5. Supports conversions of types with type params even in case of nested type params 
no matter how deep nesting is, Without the use of `TypeToken` for kotlin consumer code 
(for java consumer code `GsonConverter` is needed in case of any type params like `TypeToken`) isa.
6. building on top of `core_android` module for `Bundle` using `buildBundleGson(vararg values: Any?)`
to save any cutom object on bundle, And surely that is in addition to put & get 
values from bundle easily without the need of explicitly mentioning the key
which is already the feature of `buildGson` in `core_android` module. 

### Download ( gson & gson_annotation & gson_processor ) [▴](#gson--gson_annotation--gson_processor-modules-)
...
// ...
dependencies {
      implementation 'com.github.MohamedAlaaEldin636.MAUtils:gson:$mautils_version' // has annotation inside (opt-out).
      kapt 'com.github.MohamedAlaaEldin636.MAUtils:gson_processor:$mautils_version'
}
```

### How to use ( gson & gson_annotation & gson_processor ) [▴](#gson--gson_annotation--gson_processor-modules-)

- [Simple Use Cases](#simple-use-cases--gson--gson_annotation--gson_processor--)
- [Complex Use Cases](#complex-use-cases--gson--gson_annotation--gson_processor--)
- [All Cases](#all-cases--gson--gson_annotation--gson_processor--)

#### Simple Use Cases ( gson & gson_annotation & gson_processor ) [▴](#how-to-use--gson--gson_annotation--gson_processor--)
...
// object declaration -> maintain same instance
...
assertEquals(objectInstance, value)
```
...
// In case using a class from a kotlin code created by object declaration -> maintain same instance isa.
ObjectClassA objectClassA = ObjectClassA.INSTANCE;
String json = GsonUtils.toJson(objectClassA); // No need to specify the class as it's inferred from the instance isa.
ObjectClassA value = GsonUtils.fromJson(json, ObjectClassA.class);
assertEquals(objectClassA, value);
```

</p>
</details>

#### Complex Use Cases ( gson & gson_annotation & gson_processor ) [▴](#how-to-use--gson--gson_annotation--gson_processor--)

``` kotlin
// -- sealed classes / abstract classes / interfaces conversion -- //

// Declaration
sealed class SealedClass {
    // Note it has to be data class or object, otherwise you have to define equals fun.
...
val json = sealedClass.toJson() // OR sealedClass.toJson<SealedClass>() If hasn't been explicitly defined.
...
``` java
// -- abstract classes / interfaces / sealed classes(in case calling kotlin code) conversion -- //

// Declaration
@MASealedAbstractOrInterface
public abstract class AbstractClass {} // AbstractClass.java file.

public class Impl extends AbstractClass {
    private int integer; // with public getter/setter
    public Impl(int integer) { this.integer = integer; }
    @Override
    public boolean equals(@Nullable Object obj) {
        return obj instanceof Impl && ((Impl) obj).integer == this.integer;
    }
}
...
// And also if AbstractClass type was inside another class as a field's type
// the serialization/deserialization will still happen as expected isa.
```

</p>
</details>

#### All Cases ( gson & gson_annotation & gson_processor ) [▴](#how-to-use--gson--gson_annotation--gson_processor--)

``` kotlin
// Building on `core_android` module work with `Bundle` & using json conversions
// you can put/get cusom objects in a `Bundle` very easily.

// putting values in a bundle.
...
2. Supports **Any Type** thanks to the other module in this library which is [gson module](#gson--gson_annotation--gson_processor-modules-) module auto conversion will be done for you so you can save custom object to `SharedPreference` easily without making the conversion to/from String yourself. <br/>
...
// since Shared preference do not save null types then fallback
...
2. Since it's built on top of [shared_pref_core module](#shared_pref_core-module-) it supports **Any Type** not just the types supported by `SharedPreferences` and in addition to that it supports manual conversion in case auto conversion by [gson module](#gson--gson_annotation--gson_processor-modules-) of this library isn't fulfilling your needs.
