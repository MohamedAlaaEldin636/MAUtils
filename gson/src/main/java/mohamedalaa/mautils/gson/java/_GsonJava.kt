...
inline fun <reified E> E?.toJsonOrNull2(genericType: Type, gson: Gson? = null): String? = this?.run {
    val usedGson = gson?.addTypeAdapters() ?: privateGeneratedGson

    try { object : GsonConverter2(genericType, usedGson){}.toJsonOrNull(this) } catch (e: Exception) { null }
}
fun String?.fromJsonOrNull2(genericType: Type, gson: Gson? = null): Any? = this?.run {
    val usedGson = gson?.addTypeAdapters() ?: privateGeneratedGson

    try { object : GsonConverter2(genericType, usedGson){}.fromJsonOrNull(this) } catch (e: Exception) { null }
}

abstract class GsonConverter2(
