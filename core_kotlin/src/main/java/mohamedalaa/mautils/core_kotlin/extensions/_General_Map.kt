...
/**
 * Creates a new [Map], where each element is calculated by calling the specified [buildAction] fun
 * with index as param ranges from 0 `until` [size], the returned [Map.size] is guaranteed to be
 * like [size] if [checkSize] is true since otherwise null would return, else then [Map.size] is unknown,
 * **Note** default value of [checkSize] is false isa.
 *
 ...
 */
fun <K, V> buildMapOrNull(size: Int, checkSize: Boolean = false, buildAction: (Int) -> Pair<K, V>): Map<K, V>?
 ...
 */
fun <K, V> buildMap(size: Int, checkSize: Boolean = false, buildAction: (Int) -> Pair<K, V>): Map<K, V>
