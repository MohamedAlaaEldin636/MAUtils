 ...
 */
inline fun <reified VM : ViewModel> FragmentActivity.getViewModel(factory: ViewModelProvider.Factory? = null)
    = ViewModelProviders.of(this, factory).get(VM::class.java)

inline fun <reified FA : FragmentActivity, reified VM : ViewModel> FA.getAndroidViewModelWithFactory(vararg args: Any?): VM {
    val list = listOf(application) + args.toList()

    return getViewModel(ViewModelFactory(true, list))
}

inline fun <reified FA : FragmentActivity, reified VM : ViewModel> FA.getViewModelWithFactory(vararg args: Any?): VM {
    val list = listOf(application) + args.toList()
